...
import br.ufes.inf.nemo.marvin.research.application.GenerateBibliographyService;
...
public class GenerateBibliographyController extends JSFController {
	...
	public void init(AcademicDAO academicDAO) {
		logger.log(Level.FINE, "Initializing GenerateBibliographyController: loading researchers and creating the academic converter...");
	...
	public List<BibGenResearcher> getBibGenResearchers() {
		logger.log(Level.FINE, "Converting the set of researchers in the bibgen configuration to a list. Size should be: {0}", configuration.getResearchers().size());
	...
	public void addResearcher() {
		...
		if (selectedResearcher != null) configuration.addResearcher(selectedResearcher, null, null);
	}
	...
	public long countPublications(Academic researcher) {
		logger.log(Level.FINE, "Counting the number of publications of researcher: {0}", researcher.getName());
